...
var thisSlider, actualItemWidth, newItemWidth, activeAnimation = false, hoverSideBar = false, inter, ajaxLoader, ajaxContainer = $("#ajax-content"), reports, entitiesInReport = [], nav_history = [{type:"home", idx:"0", name: "Home"}]
;

function checkReportState(reports) {
	if (reports) {
        ...
        data: {
          ...
          "data": {"reportIdx": report.reportIdx}
        },
        success: function(res) { 
          ...
          switch (res.result) {
            case AJAX_RESULT_OK: 
            	reportData = res.data;
            	$.each(reportData.entities, function(i, v) {
            		entitiesInReport.push(v)
            	})
            	for (i = 0; i < entitiesInReport.length; i++ ) {
		            if ("#"+entitiesInReport[i].cadaptIdx) {
		              $("table tr#"+entitiesInReport[i].cadaptIdx).find(".select i").removeClass('fa-square-o fa-check-square-o').addClass('inreport fa-file-text-o')
		            }
		          }
              break;
...
$(window).load(function(){
	// Override the _=TIMESTAMP that jQuery adds to $.ajax calls with "cache: false" option defined
	...
	var disableBack = function() {
		...
		if ( location.hash == '#no-back' ) {
		  ...
		  window.onhashchange = function() {
		    ...
		    if ( location.hash == '#no-back' ) {
		      alert("Browsing 'back' capability coming soon")
		      if ( history_api ) history.pushState(null, '', '#stay')
		      else location.hash = '#stay'
		    }
		  }
	...
	function getReports() {
      ...
      dataType: 'json',
	    url: g_getURLHeader() + '/controller/cAjax.php',
	    data: {
        "action":"retrieveAllEntitiesReports",
        "data": null
	    },
	    success: function(res) {
        // console.log(res);
        switch (res.result) {
          case AJAX_RESULT_OK:
          	listUserReports(res.data)
          	reports = res.data
          	checkReportState(reports)
            // console.log("OK!");
            break;
          case AJAX_RESULT_EXCEPTION: //Exception error
            alert("Exception error: " + res.thrown);
  ...
  function listUserReports(reports) {
  	if (reports.length > 0) {
  		// console.log('list reports > 0');
  		$(".user-report").remove();
  		$.each(reports, function(index, aReport) {
  		  $('<li class="user-report"><a id="'+aReport.reportIdx+'" href="#" title="'+aReport.reportName+'"><i class="fa fa-file-text-o fa-fw icon"></i> '+aReport.reportName+'<i class="fa fa-remove action delete"></i></a></li>').insertBefore($('.create-report').parent('li'))
  		});
  	} else {
  		// console.log('no reports');
  	}
  ...
  function submitCreateReport() {
  	...
  	var reportNotes = $("#report-notes").val().trim();
	  $(".select i.fa-check-square-o").parents("tr").each(function() {
	  	var entityType = $(this).data("type"), cadaptIdx = $(this).attr("id"),
	  		entity = {"entityType":entityType, "cadaptIdx":cadaptIdx};
	...
	function submitUpdateReport(reportIdx) {
		var entity, entities = []
	...
	$(document).on("click", ".submitCreateReport", function() {
	  submitCreateReport()
	  $("#panel-config").find('.modal-footer .btn-primary').removeClass("submitCreateReport")
	});

	$(document).on("click", "#item-actions .user-report a", function() {
	  submitUpdateReport($(this).attr("id"))
	});

  $(document).on("click", ".main-navigation-menu .user-report a", function() {
  	var reportIdx = $(this).attr('id');
    var prev = {type:"report", idx: reportIdx, name: $(this).text()};
    nav_history.push(prev)
  	var url = g_getURLHeader() + '/view/vUserReport.php?idx='+reportIdx
		ajaxLoader(url, ajaxContainer, nav_history)
  });

	$(document).on("click", "a.alarmDetailsLink", function(event) {
		event.preventDefault()
    event.stopPropagation()
    var prev = {type:"alarm", idx: $(this).data('idx'), name: $(this).text()};
    nav_history.push(prev)
    var url = g_getURLHeader()+'/view/vAlarmDetails.php?idx='+$(this).data('idx')
    ajaxLoader(url, ajaxContainer, nav_history)
	})

	$(document).on("click", "a.hostDetailsLink", function(event) {
		event.preventDefault()
    event.stopPropagation()
    var prev = {type:"host", idx: $(this).data('idx'), name: $(this).text()};
    nav_history.push(prev)
    var url = g_getURLHeader()+'/view/vHostDetails.php?idx='+$(this).data('idx')
    ajaxLoader(url, ajaxContainer, nav_history)
	})

	$(document).on("click", "a.notifDetailsLink", function(event) {
		event.preventDefault()
    event.stopPropagation()
    var prev = {type:"notif", idx: $(this).data('idx'), name: $(this).text()};
    nav_history.push(prev)
    var url = g_getURLHeader()+'/view/vNotifDetails.php?idx='+$(this).data('idx')
    ajaxLoader(url, ajaxContainer, nav_history)
	})

  $(document).on("click", "a.homeLink", function(event) {
    event.preventDefault()
    event.stopPropagation()
    url = g_getURLHeader()+'/view/vDashboard.php'
    nav_history = [{type:"home", idx:"0", name: "Home"}]
    ajaxLoader(url, ajaxContainer, nav_history);
  })
	
	// --------- Modal Opening ----------- //
	$(".main-wrapper").on('click', '#countryCode', function(event) {
		event.preventDefault();
		$("#panel-config").find('.toggle-content').addClass('hidden').end().find(".modal-title").text('Geo Location of Unmonitored Host 65.55.252.71').end().find('.geolocate').removeClass('hidden').end().modal();
	});

	$(".main-wrapper").on('click', '#threat-feed', function(event) {
		event.preventDefault();
		$("#panel-config").find('.toggle-content').addClass('hidden').end().find(".modal-title").text('External Detection(s)').end().find('.external-data').removeClass('hidden').end().modal();
	});

	$(".main-wrapper").on('click', '.create-report', function(event) {
		event.preventDefault();
		$("#panel-config").find('.modal-footer .btn-primary').removeClass().addClass("btn btn-primary submitCreateReport").end().find('.toggle-content').addClass('hidden').end().find(".modal-title").text('Create New Report').end().find('.create-report').removeClass('hidden').end().modal();
	});

	$(document).on('click', '.action.delete', function(event) {
		event.preventDefault();
		var reportIdx = $(this).parent("a").attr("id")
		$("#panel-config").find('.modal-footer .btn-primary').removeClass().addClass("btn btn-primary confirmDeleteReport").attr("data-idx",reportIdx).text("Delete").end().find('.modal-footer .btn-default').text("Cancel").end().find('.toggle-content').addClass('hidden').end().find(".modal-title").text('Confirm Delete Report').end().find('.delete-report').removeClass('hidden').end().modal();
	});

	$(document).on('click', '.confirmDeleteReport', function(event) {
		event.preventDefault();
		var reportIdx = $(this).data("idx");
		console.log("delete report idx: "+reportIdx);
		deleteReport(reportIdx);
		$("#panel-config").find('.modal-footer .btn-primary').removeClass().addClass("btn btn-primary").text("Save Changes").end().find('.modal-footer .btn-default').text("Close").end().modal("hide");
		$(".main-navigation-menu .user-report").find("#"+reportIdx).parent("li").remove();
		var url = g_getURLHeader()
    ajaxLoader(url, ajaxContainer)
	});


	// --------- Row Selections ----------- //
	$(document).on('click', '#item-actions .back', function(event) {
		event.preventDefault();
		clearCheckboxes();
	});

	function clearCheckboxes(){
		$(".select i.seen").removeClass("fa-check-square-o").addClass('fa-square-o')
		$(".select i.unseen").removeClass("fa-check-square-o").addClass('fa-circle')
	...
	$(document).on({
		mouseenter: function () {
		  if ($(this).hasClass('fa-circle')){
		  	$(this).removeClass('fa-circle').addClass('fa-square-o');
			}
		},
		mouseleave: function () {
		  if ($(this).hasClass('fa-check-square-o')){
		  	$(this).stop(true, true)
				...
				$(this).removeClass('fa-square-o').addClass('fa-circle');
			}
		}
	}, "tbody .select i.unseen");

	// --------- Tooltips init for Bootstrap -------------------------------------------------------- //
	
	$('[data-toggle="tooltip"]').tooltip()

	// --------- End Template Interaction Snippets -------------------------------------------------------- //

	if($('body').hasClass('pageload')){
	...
	if( !$('body').hasClass('login') ) {
		var timeout_sec = 60 * 60; //60 minutes
		var lastUpdate = new Date().getTime();

		//If not the login page, do session timers
		$("body").on("click mousemove keyup keydown keypress",function() {
			lastUpdate = new Date().getTime();
		});

		var sessionCheck = function() {
			var timeNow = new Date().getTime();
			var timePassed = Math.floor((timeNow - lastUpdate) / 1000);
//			console.log("lastUpdate: " + lastUpdate + ", timeNow: " + timeNow, ", timePassed: " + timePassed +", timeout_sec: " + timeout_sec);
			if (timePassed > timeout_sec) {
				clearInterval(sessionCheckInterval);
				alert("Your session has timed out. You will now be redirected to the login screen.");
				window.location = window.location.origin + g_baseURI + "/controller/cLogout.php";
			}
		}
		var sessionCheckInterval = window.setInterval(sessionCheck, 1 * 1000); //Run every second (1 second * 1000ms)
		...
		//  	if (g_iScriptedDemo == 1) { //For scripted demo, all indicators are green. yay! :)
		//  		$("#systemStatus .indicator").removeClass().addClass('indicator green');
		//  	} else {
		// 	  $.ajax({  
		//       type: "GET", 
		//       dataType: 'json', 
	 ...
	 //            	if (result.system == 1) { //Good
	 //            		$("#systemStatus .indicator").removeClass().addClass('indicator green');
	 //            	} else { //Not so good
	 //            		$("#systemStatus .indicator").removeClass().addClass('indicator red');
	 //            	}
	            	
	 //            	$("#csuStatus .indicator").removeClass().addClass('indicator ' + ((result.csu == 1)?"green":"red"));
	 //            	$("#scpStatus .indicator").removeClass().addClass('indicator ' + ((result.scp == 1)?"green":"red"));
	 //            	$("#acsStatus .indicator").removeClass().addClass('indicator ' + ((result.acs == 1)?"green":"red"));
	 ...
	 //        }
		//     }); //End of ajax call
		//  	} //End of else of (if (g_iScriptedDemo == 1))
		//  	return;
		// }(); //End of function updateStatuses - Run immediately
		
		// window.setInterval(updateStatuses, 60 * 1000); //Run again every minute (60 seconds * 1000ms)		
	}
	
	//Set system status mouseover/mouseout events
	$("#systemStatus").on("mouseover", function() {
		$("#systemStatusDropdown").show();
	});
	$("#systemStatus").on("mouseout", function() {
		$("#systemStatusDropdown").hide();
	});
...
var Main = function() {"use strict";
	...
	var runNavigationMenu = function() {
		if($("body").hasClass("single-page") == false) {
			...
			$('.main-navigation-menu > li.active').addClass('open');
			$('.main-navigation-menu > li a').not(".create-report").on('click', function(e) {
				e.preventDefault();
				var $this = $(this);
	...
	ajaxLoader = function(url, element, nav_history) {
		...
		if($body.hasClass("sidebar-mobile-open")) {
			...
			extendOptions = {
				complete: function() {
					...
					loadPage(url, element, nav_history);
		...
		function loadPage(url, element, nav_history) {
			$.ajax({
				type: "GET",
				url: url,
				dataType: "html",
    ...
    function createBreadcrumbs(nav_history) {
      var position = 0, nav_history_length = nav_history.length
      $.each(nav_history, function(index, val) {
        var nav_icon_class, nav_link_class
        switch(val.type) {
          case "home":
          default:
            nav_icon_class = "fa fa-home"
            nav_link_class = "homeLink"
            break;
          case "alarm":
            nav_icon_class = "fa fa-bell-o"
            nav_link_class = "alarmDetailsLink"
            break;
          case "notif":
            nav_icon_class = "fa fa-exclamation-triangle"
            nav_link_class = "notifDetailsLink"
            break;
          case "host":
            nav_icon_class = "fa fa-laptop"
            nav_link_class = "hostDetailsLink"
            break;
          case "report":
            nav_icon_class = "fa fa-file-text-o"
            nav_link_class = "userReportLink"
            break;
        }
        var $nav_li = $( '<li><a class="'+nav_link_class+'" data-position="'+position+'" data-idx="'+val.idx+'" href="#" title="'+val.name+'"><i class="icon '+nav_icon_class+'"></i><span class="title">'+val.name+'</span></a></li>' )
        $(".breadcrumb").append($nav_li)
        position++
      });
      ...
      $(".breadcrumb").on('click', 'li a', function(event) {
        ...
        var remove = clicked+1
        console.log(nav_history);
        console.log("clicked: "+clicked);
        console.log("length: "+nav_history_length);
        console.log("subtraction: ")
        console.log(nav_history_length - clicked);
        nav_history.splice(-remove, nav_history_length)
        console.log(nav_history);
      });
    }
	};
